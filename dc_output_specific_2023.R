...
individual_old <- read_excel(annex_path, 
  ...
  rename(fi_idcou = fi_id_cou)


# get the database id (fi_id) in the created output (to distinguish update from/new data)
data_individual <- data_individual %>% select(-sai_emu_nameshort)

#replace NaN with NA (otherwise they won't be recognized as duplicates)
data_individual[sapply(data_individual, is.nan)] <- NA

#check if df columns are similar  
setdiff(names(individual_old), names(data_individual))
setdiff(names(data_individual), names(individual_old))

# create tables for common, updated and new data
individual_deleted <- individual_old
individual_new <- data_individual
...
sai_info_new <- anti_join(sai_info, sai_old, by = "sai_name")
if (nrow(sai_info_new) == 0){sai_info_new <- data.frame("NO_NEW_SERIES" = "")}

#####------------------------- 8. PRINT CSVs -------------------------#####

write.table(individual_deleted, file = "output_data/data_individual/2023_individual_deleted.csv", row.names = FALSE, sep = ";")
...
write.table(sai_info_new, file = "output_data/sai_info/2023_sai_info_new.csv", row.names = FALSE, sep = ";")
